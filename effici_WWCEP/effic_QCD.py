...
multiRP_QCD = np.concatenate(  (QCD , EfficienciesStudies(QCD).reshape(-1,1) ), axis = 1 )
...
with h5py.File( 'DataSet_multiRP_QCD.h5', 'w') as f:
   dset = f.create_dataset( 'dados', data = DataFrame_multiRP_QCD[select_columns_MC] )

print( DataFrame_multiRP_QCD[select_columns_MC] )
